...
print(pow(3, 2))  # Какой pow()? Из math или my_math?
```
...
В ранних версиях Python (до 3.3) файл __init__.py был обязателен, чтобы Python распознавал папку как пакет (модуль). Без него импорт просто не работал. Однако начиная с Python 3.3, появился новый механизм — implicit namespace packages (неявные пространственные пакеты), и теперь:

Файл `init.py` не обязателен, если тебе не нужно выполнять код при импорте или явно управлять структурой пакета.
