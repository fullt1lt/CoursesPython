...
Свойства архитектуры, которые зависят от ограничений, наложенных на `REST-системы`:
 ...
 4. `Uniform Interface`. Единый интерфейс определяет интерфейс между клиентами и серверами. Это упрощает и отделяет архитектуру, которая позволяет каждой части развиваться самостоятельно. Четыре принципа единого интерфейса:
    ...
    4.1) **Identification of resources**(основан на ресурсах). В `REST` ресурс — это любая сущность, которую можно однозначно назвать и получить с сервера.
    То есть — это не страница и не конкретный файл, а понятие, описывающее какой-то объект в системе. Например: пользователь, статья, категория, изображение, прогноз погоды. В `REST` всё вращается вокруг этих ресурсов, а не вокруг действий. Чтобы клиент мог обратиться к конкретному ресурсу, ему нужен адрес.
    В `REST` таким адресом служит `URI` (**Uniform Resource Identifier)** — обычно в виде URL.
    Примеры ресурсов и их URI:
    | Ресурс                    | URI                | Комментарий              |
    | ------------------------- | ------------------ | ------------------------ |
    | Список всех пользователей | `/api/users/`      | Это *коллекция ресурсов* |
    | Конкретный пользователь   | `/api/users/15/`   | Это *один ресурс*        |
    | Список всех статей        | `/api/articles/`   | Коллекция статей         |
    | Конкретная статья         | `/api/articles/7/` | Ресурс — статья с ID=7   |

    > URI не должен зависеть от состояния ресурса. Например, если у статьи изменился заголовок или категория, её URI не меняется — она остаётся по адресу `/api/articles/7/`.


    4.2) **Manipulation of resources through representations**. (Манипуляции над ресурсами через представления). Представление в REST используется для выполнения действий над ресурсами. 
    В `REST` ресурс — это абстрактная сущность (например, “пользователь”, “статья”, “категория”).А представление — это конкретный способ, как этот ресурс выражен при передаче по сети.
    > Иными словами: **Ресурс** — это “объект” внутри системы, **Представление** — это “данные”, которыми клиент и сервер обмениваются, описывая этот объект.
    
    **Пример**
    Ресурс: пользователь с ID=1(в базе это объект Python, запись в таблице, не JSON)
    Его представление на сервер может быть:
    ...
    ```
    Клиент и сервер могут использовать разные представления одного и того же ресурса, в зависимости от потребностей. Например, мобильное приложение может запрашивать JSON, а веб-браузер — HTML.
    ...
    4.4) **HATEOAS** (hypermedia as the engine of application state). Клиент должен взаимодействовать с REST API через гиперссылки, которые сервер сам возвращает в ответах. То есть клиент не должен заранее знать все маршруты API, а должен находить, куда идти дальше, прямо в ответах от сервера.
    
    **Пример без HATEOAS (обычный API)**
      Клиент знает все адреса заранее:
    ```url
    GET /api/articles/
    ```
    ...
    ```json
    [
      {"id": 1, "title": "Что такое REST"},
      {"id": 2, "title": "HATEOAS — последняя буква REST"}
    ]
    ```

    Чтобы перейти к конкретной статье, клиент сам “додумывает”:

    ```url
    /api/articles/1/
    ```

    > Это нарушает принцип HATEOAS, потому что клиенту нужно знать внутреннюю структуру API заранее.

    **Пример с HATEOAS**

    Теперь сервер сам подсказывает, куда можно перейти:
    ```json
    [
      {
        "id": 1,
      ...
      {
        "id": 2,
        "title": "HATEOAS — последняя буква REST",
        "url": "/api/articles/2/"
      }
    ]
    ```

    **Как это выглядит в “глобальном” API**

    Клиент начинает с корня `API`:
    ...
    Теперь клиент знает всё, что поддерживает `API`, и может *“путешествовать”* по нему как по сайту. Это делает `API` само-документируемым и независимым от заранее жёстко прописанных маршрутов.
 ...
 6. **Code-On-Demand (опционально)**. Принцип Code on Demand означает, что сервер может передавать клиенту исполняемый код, который клиент может выполнить у себя для расширения или изменения функциональности.
 
 > Иными словами: Сервер может *“на лету”* научить клиента чему-то новому.

 В контексте `REST API` этот принцип встречается редко, потому что современные `API` почти всегда обмениваются данными `(JSON)`, а не исполняемым кодом.
 Но теоретически — `REST` допускает это.
...
Каждое поле сериалайзера (`CharField`, `DecimalField`, `BooleanField`) напрямую соответствует полю модели `Product`. Когда мы передаём объект модели в сериалайзер, он создаёт словарь с этими полями, а при приёме `JSON` — наоборот, превращает словарь в объект `Python` с соответствующими типами данных.
...
Здесь сериалайзер не *“связан”* напрямую с моделью.Мы полностью управляем тем, какие поля попадут в сериализацию и как они будут называться.

2. **Метод create()**

Этот метод вызывается, когда сериалайзер используется для создания нового объекта модели.
...
Аргумент `validated_data` — это словарь с уже проверенными и приведёнными к нужным типам данными. `DRF` автоматически формирует этот словарь после валидации (после вызова `serializer.is_valid()`).

> По факту метод is_valid() проверяет данные, и если они корректны, вызывает create() с этими данными. Все работает так же, как и в Django-формах. только здесь мы создаём объект модели вручную.

Метод `create()` создаёт новый объект Product в базе и возвращает его.

3. **Метод update()**

Этот метод вызывается, когда сериалайзер используется для обновления существующего объекта.
Он принимает два аргумента:
- `instance` — сам объект модели, который нужно обновить;
- `validated_data` — словарь с проверенными данными, пришедшими от клиента.
...
Здесь для каждого поля мы берём новое значение из `validated_data`, а если его нет — оставляем старое.
После обновления вызывается `instance.save()`, чтобы сохранить изменения в базе.

#### Пример использования сериализатора

Рассмотрим это на примере интерактивной оболочки Django (`python manage.py shell`).

```python
from my_app.api.serializers import ProductSerializer
from my_app.models import Product
...
Мы передали объект модели Product в сериалайзер, и он превратил его в словарь `Python`, готовый к преобразованию в `JSON`.

#### Десериализация (JSON → объект)

Теперь посмотрим обратный процесс — когда клиент присылает нам данные.
...
serializer.is_valid()       # проверяем данные и формируем validated_data
product = serializer.save() # вызывает create(validated_data)
print(product)         # новый объект создан в базе
```

DRF:

- Проверяет типы данных (например, price должен быть числом);
- Создаёт `validated_data` — словарь с проверенными и преобразованными данными;
- Понимает, что мы не передавали instance, значит, нужно вызвать `create()`;
- Создаёт новый объект модели в базе.

#### Обновление объекта (`PUT` / `PATCH`)

```python
product = Product.objects.first()
data = {"price": "24999.99", "in_stock": False}

serializer = ProductSerializer(product, data=data, partial=True)
serializer.is_valid()
product = serializer.save()  # вызывает update(instance, validated_data)
```

Мы передаём в сериалайзер не только данные, но и сам объект модели (`product`). Благодаря этому DRF понимает, что нужно не создавать новый, а обновить существующий объект.

Аргумент `partial=True` означает, что обновлять можно не все поля сразу.
...
| ---------------- | -------------------------------------------------------------- |
| `initial_data`   | исходные данные, пришедшие от клиента                          |
| `validated_data` | проверенные и приведённые к типам значения после валидации     |
| `instance`       | объект модели, который сериалайзер должен создать или обновить |
| `create()`       | вызывается при создании нового объекта                         |
| `update()`       | вызывается при обновлении существующего объекта                |
| `save()`         | автоматически решает, какой из методов вызвать                 |
...
Внутренний класс Meta сообщает сериалайзеру:

- `model` — с какой моделью он работает;
- `fields` — какие именно поля брать из модели (аналогично `fields` в `ModelForm`).
...
Cериалайзер автоматически включит все поля модели.

`ModelSerializer` сам создаст методы `create()` и `update()`, используя стандартные методы модели.

> Нем не нужно писать их вручную, если не требуется кастомная логика(Например кастомная валидация или обработка полей).

Проверим его работу в интерактивной оболочке(`python manage.py shell`):

#### Пример сериализации с ModelSerializer

```python
from my_app.models import Product
from my_app.api.serializers import ProductModelSerializer

product = Product.objects.create(
    name="Монитор",
    description="Игровой 27 дюймов",
    price=12999.99,
    in_stock=True
)
...
#### Пример десериализации с ModelSerializer
...
serializer.is_valid()
product = serializer.save()  # вызывает create()
print(product)  # новый объект создан в базе
```

DRF:

- Проверил корректность типов (`price` — число, `in_stock` — логическое);
- Сформировал `validated_data`;
- Вызвал внутренний create(`validated_data`);
- Создал новый объект `Product`.

> Методы `validate` и `save` работают одинаково, как и в предыдущем примере с обычным `Serializer`. Их можно переопределить при необходимости. При необходимости вы можете добавить кастомную логику валидации или обработки полей, переопределив соответствующие методы.
...
Поскольку мы передали объект `product`, сериалайзер вызовет метод `update(instance, validated_data)` и обновит только цену (остальные поля останутся без изменений, так как указан `partial=True`).
...
| Характеристика                 | `Serializer`                            | `ModelSerializer`                    |
| ------------------------------ | --------------------------------------- | ------------------------------------ |
| Создание полей                 | Задаются вручную                        | Генерируются автоматически из модели |
| Методы `create()` / `update()` | Нужно писать вручную                    | Генерируются автоматически           |
| Гибкость                       | Максимальная, но больше кода            | Оптимален для CRUD на моделях        |
| Подходит для                   | Сложных и нестандартных структур данных | Быстрой сериализации моделей         |
| Валидация                      | Полностью кастомная                     | Автоматическая на основе модели      |
...
Если вычисляемое значение не реализовано в модели, или если логика зависит от контекста (например, от текущего пользователя), удобнее использовать `SerializerMethodField`.
...
class ProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = ['id', 'name', 'price', 'discount_percent', 'discounted_price', 'is_expensive']

    def get_discounted_price(self, obj):
        """Вычисляем цену со скидкой прямо в сериалайзере"""
        discount = (obj.discount_percent / Decimal('100')) * obj.price
        return obj.price - discount

    def get_is_expensive(self, obj):
        """Пример вычисляемого логического поля"""
        return obj.price > 5000
```

Теперь у нас два вычисляемых поля:

- `discounted_price` — вычисляется через метод `get_discounted_price()`;
- `is_expensive` — возвращает `True`, если цена больше 5000.

DRF автоматически вызывает метод `get_<имя_поля>` для каждого `SerializerMethodField`.

Результат:

```python
product = Product(name="Клавиатура", price=2500, discount_percent=20)
serializer = ProductSerializer(product)
print(serializer.data)
```
...
#### Чтение сложных вложенных структур с ModelSerializer

На практике часто нужно передавать не только *“плоские”*данные, но и вложенные — например, у товара должна отображаться категория,а у категории — список связанных товаров.

В таких случаях мы используем вложенные сериалайзеры `(nested serializers)`. Это позволяет описывать связи между моделями прямо в структуре ответа `API`.

**1. Пример моделей**

Создадим две связанные модели — `Category` и `Product`. Каждый товар относится к одной категории `(связь ForeignKey)`,а у категории может быть множество товаров.
...
Мы хотим сформировать API, которое отдаёт список продуктов вместе с их категориями, но при этом не делает десятки лишних SQL-запросов.

Также хотим иметь возможность получать данные *“вложенно”* — например, получить категорию и список всех её товаров.

**3. Первый вариант — вложенный сериалайзер (чтение категории внутри продукта)**

Сначала создадим сериалайзер для Category, а затем встроим его внутрь ProductSerializer.
...
Теперь при сериализации продукта `DRF` автоматически вставит объект категории в виде словаря.
...
**4. Второй вариант — обратная вложенность (список продуктов внутри категории)**

Теперь попробуем сделать обратную ситуацию: сериалайзер `Category` будет содержать список всех связанных продуктов.

```python
class ProductMiniSerializer(serializers.ModelSerializer):
    """Мини-сериалайзер без описания, только основные данные"""
    class Meta:
        model = Product
        fields = ['id', 'name', 'price']
...
```python
category = Category.objects.prefetch_related('products').first()
...
```json
{
  ...
  "name": "Электроника",
  "products": [
    {"id": 1, "name": "Монитор", "price": "15999.99"},
    {"id": 2, "name": "Клавиатура", "price": "2999.99"},
    {"id": 3, "name": "Мышь", "price": "1999.99"}
  ]
}

```

**5. Расширенный пример с получением всех данных**
...
[
  ...
  },
  {
    ...
    "products": [
      {"id": 4, "name": "Холодильник", "price": "49999.99"},
      {"id": 5, "name": "Стиральная машина", "price": "39999.99"}
    ]
  }
]
```
...
Сегодня мы рассмотрели, как работать с вложенными сериализаторами в `Django REST Framework`. Этот подход позволяет легко и удобно представлять связанные данные в `API`, делая его более информативным и удобным для клиентов. На следующей паре мы научимся создавать представления `(views)` для наших сериализаторов и моделей, чтобы полностью реализовать `CRUD` операции в нашем `API`.
