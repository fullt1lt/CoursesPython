...
Если хочется создать проект в текущей папке то можно добавить в конец `.`
```bash
django-admin startproject my_project .
```
Тогда `manage.py` будет лежать рядом с вашим проектом в текущей директории, и не будет большой вложенности.
В курсе мы будем использовать первый вариант.
