...
Эти паттерны помогают гибко и удобно создавать объекты.

#### Простая фабрика (Simple Factory)

Позволяет скрыть логику создания объектов внутри одного метода.
...
- Когда нужно создать объект, но не хочется прописывать детали его создания в основном коде.
- Когда объекты создаются одинаковым способом, но могут отличаться параметрами.
- Например, при разработке игр для создания различных типов персонажей.

#### Фабричный метод (Factory Method)

Позволяет подклассам определять, какие объекты создавать.

**Пример из жизни:**

Вы заходите в разные рестораны, но в каждом подают разные блюда. Китайский ресторан готовит лапшу, а итальянский – пасту. Но меню у всех одно – это и есть фабричный метод.
...
Позволяет объекту с одним интерфейсом работать с другим интерфейсом.
...
Объединяет сложную систему в один удобный интерфейс.

**Пример из жизни:**

Чтобы включить телевизор, нужно вставить вилку, нажать кнопку питания, выбрать вход HDMI... Но с пультом (фасадом) всё проще – вы просто нажимаете одну кнопку.

**Где применять?**

- Когда нужно скрыть сложную логику системы за простым интерфейсом.
- Когда у системы много зависимостей и её нужно упростить.
- В разработке веб-приложений, где фронтенд взаимодействует с сервером через единый API.

### Поведенческие паттерны

Эти паттерны управляют взаимодействием между объектами и их поведением.

#### Команда (Command)

Позволяет превращать запросы в объекты, что упрощает управление операциями.
...
- Когда нужно инкапсулировать запросы в виде объектов.
- В разработке undo/redo систем (например, в текстовых редакторах).
- В играх, где у игрока есть различные действия (атака, защита, перемещение).

#### Наблюдатель (Observer)

Объекты автоматически обновляются при изменении других объектов.

**Пример из жизни:**

Вы подписались на YouTube-канал. Как только выходит новое видео, вы получаете уведомление – это и есть паттерн "наблюдатель".

**Где применять?**

- Когда один объект должен автоматически уведомлять другие о своих изменениях.
- В GUI-интерфейсах (например, обновление отображения данных при изменении модели).
- В играх, когда персонаж может получать обновления о состоянии окружающей среды.

### Итог

- Паттерны – это не "волшебная таблетка", но они делают код проще и понятнее.
- Каждый паттерн решает конкретную проблему.
- Зная паттерны, вы сможете писать профессиональный и чистый код.