...
`len()` не умеет работать с int, конвертируйте int в str прежде, чем использовать в len (к примеру для вычисления сколько чисел в int).
...
phone_number = 420123123123
print(len(phone_number)) # выбросит TypeError
print(len(str(phone_number))) # 12 (количество символов в номере)
```
